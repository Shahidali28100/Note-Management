...
- [ ] 4.5 CI verification (plan §0/§4 — **still not confirmed from this environment**): push this branch and watch the `backend` CI job (now `ubuntu-latest` + `mssql/server` container) go green end-to-end — restore, migrate, build, test. Deliberately left undone here — pushing is outside `/implement`'s scope without explicit user go-ahead (root `CLAUDE.md` Permission Model: `git push` requires `[y/n]`). This remains the authoritative outstanding check for the container-based CI change; everything else in this ticket has been verified against a real local SQL Server Express instance.
